  ...
  function run(filename) {
    ...
    for (var i=0, len=lines.length; i<len; ++i) {
      ...
      // 코드를 분석합니다.
        ...
        if (left == null) {
          // 니모닉만으로 구성된 명령에 대한 조건 분기 (피연산자 0개)
          if (mne == 'ret') {
            log('ret routine');
          }
          else {
        ...
        // 이외의 경우 일단 예외 처리합니다.
          ...
          throw new RunnerException('invalid left operand', {
            lineNumber: i, lineString: line, operand: left
          });
        }
        ...
        // 다음 토큰이 없다면 분석이 끝납니다.
        if (right == null) {
          // 니모닉, 피연산자 구성의 명령에 대한 조건 분기 (피연산자 1개)
          if (mne == 'push') {
            log('push routine');
          }
          else if (mne == 'pop') {
        ...
        if (right != ',')
          throw new RunnerException(
            'syntax error; right operand must be after comma(,)', {
              lineNumber: i, lineString: line, operand: right
            });
        ...
        // 니모닉, 피연산자 구성의 명령에 대한 조건 분기 (피연산자 2개)
        if (mne == 'mov') {
          log('mov routine');
        }
        else {
